...
**State as of 2026-07-22 (post RUN 14) — BREAKTHROUGH: `p.pcie_init() -> 0`:**
- m1n1 `8a569ad` enrolled and verified (`require-build OK`); the full breadcrumb trail printed: shared init + rc handshake completed cleanly with the phy-ip tunables skipped — the first completed C init since 2026-07-10 (`logs/14/run.log:195-212`). The 6b277bc ordering bug is confirmed fixed.
- **Ports stuck BUSY = pcie_up_1 parity, a non-event:** both ports failed the idle poll (`LINKSTS BUSY` bit 2, 250 ms → `continue` since m1n1 `7a1a083`), but pcie_up_1's own post-init LINKSTS (`0x8300020c`/`0x83000204`) had BUSY set too — the known-good boot behaved identically (its console lines were buffered/unlogged). Port idle/LTSSM is a later, separate problem (CLKREQ#/refclk/endpoint axis).
- **phy_ip is still locked immediately after `pcie_init`:** the straight-to-pll post-init apply wedged at `0x497040038`. And on 2026-07-11 — the only boot where phy_ip ever decoded — ports were ALSO stuck BUSY, so per-port completion is NOT the ungate. That boot's difference: **between `pcie_init` and its successful 6.g it re-ran the entire Phase F shared sequence** (pmgr enable, axi2af, phy tunables, a SECOND CLK0/CLK1 handshake, RESET clear, marker). **Corrected hypothesis: the post-init shared-sequence re-pass is the phy_ip ungate.** Full findings in `Scripts/m1n1/logs/14/findings.md`.

**RUN 15 plan (2026-07-22): the 2026-07-11 recipe — Phase F after `pcie_init`. No reflash.** `./Scripts/m1n1/perstn-run.sh 15` (`--preinit-probe --tier3 --t8140-replay-post-init --require-build=rc1-59-g`): after `pcie_init` returns, an early tier-1 dump (port LINKSTS at last), then `probe_phaseF_t8140_replay` in native order — steps 1→6.f re-pass, 6.g (29 pll entries python per-entry), 6.h (auspma, port-1 slice filtered), 7-10 — then tier-3 dumps (Tier 3a phy_ip harvest), LTSSM kick, ECAM walk. Matrix: Phase F completes → best state ever, NIC vendor/device ID in ECAM = goal; ports still BUSY after kick → RUN 16 = LTSSM/link-training (CLKREQ#/refclk) from a fully-tuned controller. 6.g wedges even post-replay → 2026-07-11 reproduced except m1n1 binary internals → diff d664bd9-era port-body pre-idle phy writes (pcie.c:836-843 `clear32 0x10` / `set32 0x200/0x400`) vs `8a569ad`, or bisect. Earlier Phase F step wedges → its label names it.

**State as of 2026-07-22 (post RUN 15) — golden-state parity confirmed; recipe still untested:**
- `p.pcie_init() -> 0` again; **the early tier-1 dump proved the post-init state is register-identical to pcie_up_1's golden dump on all 7 compared registers** (LINKSTS `0x8300020c`/`0x83000204`, PHY_CTRL `0x2300066f`, phy+4 `0x30`, PHYCMN `0x80300001`). The current boot reaches exactly the state from which 2026-07-11 decoded phy_ip.
- Two tooling interactions: (1) the Phase F post-init replay silently SKIPPED — its entry check (perstn.py:3168) needs `phaseD_gate151_active`, a python-side flag only `--gate-poke` sets, which RUN 15 dropped; (2) the unconditional Tier 3a phy_ip harvest then wedged the boot at `read32(0x497040000)`, costing the kick/ECAM sections. Full findings in `Scripts/m1n1/logs/15/findings.md`.
- Fixes (python-side, no reflash): post-init PMGR gate verification sets the Phase F precondition from hardware truth; Tier 3a is now gated on `phaseF_shared_up`.

**RUN 16 plan (2026-07-22): RUN 15 with the interactions fixed.** Same flags (`--preinit-probe --tier3 --t8140-replay-post-init --require-build=rc1-59-g`), no reflash. Matrix unchanged: Phase F post-init → 6.g (29 pll) → 6.h (auspma filtered) → 7-10 → `PHASE F SUCCESS` → Tier 3a harvest → LTSSM kick → ECAM walk (**NIC vendor/device ID = goal**); 6.g wedge → diff the d664bd9-era port-body pre-idle phy writes (pcie.c:836-843 `clear32 0x10` / `set32 0x200/0x400`) vs `8a569ad` or bisect; earlier-step failure → labeled, and the boot now survives to harvest everything else.
